...
importing paper-specific concepts, data assumptions, or pipeline
structure.

The result should feel like a natural extension of the current `bosc`
API:
...
3.  Make grouped methods work for any binned behavioral series, not only
    SOA data.
4.  Prefer a small number of orthogonal verbs over many convenience
    wrappers.
...
These functions operate on trial tables and return trial tables or
binned tables.
...
- [`align_time_bins()`](https://bbuchsbaum.github.io/bosc/reference/align_time_bins.md)
  maps observed timing values onto a target grid.
- [`residualize_trials()`](https://bbuchsbaum.github.io/bosc/reference/residualize_trials.md)
  removes nuisance structure without hard-coding any paper covariates.
- [`aggregate_by_bin()`](https://bbuchsbaum.github.io/bosc/reference/aggregate_by_bin.md)
  converts trial tables into regularized per-subject series.

These should accept bare vectors or data-frame columns by standard R
conventions, but the first implementation can stay simple and explicit
if needed.

### 2. Grouped Series Object

Introduce a lightweight internal convention for binned subject-level
data:
...
This does not need a formal S3 class in the first pass, but the
structure should be consistent enough that later methods can dispatch on
it cleanly.

### 3. Grouped Spectral API

These functions provide the generic equivalent of the paper’s grouped
FFT logic.
...
This generalizes the paper’s wavelet/TFA analyses without carrying over
its manuscript-specific plotting layer.
...
Cluster correction should reuse
[`detect_clusters()`](https://bbuchsbaum.github.io/bosc/reference/detect_clusters.md)
rather than reimplementing image-label logic from the paper code.
...
These should align with the semantics of
[`phase_at_events()`](https://bbuchsbaum.github.io/bosc/reference/phase_at_events.md)
and current circular-stat helpers.

## Null Model Design

The grouped-analysis layer should use a shared null-model interface
rather than embedding separate permutation logic in each function.
...
- `shuffle_labels` is appropriate when labels are exchangeable within
  strata.
- `circular_shift` is appropriate for preserving within-series
  autocorrelation.
- `ar1` is valuable, but should be added only after the generic null
  interface is stable.

The `AR1_control` script in `MBO_shared` should be treated as a
conceptual reference only, not as source code to port directly.
...
If the grouped APIs land, the package should also expose matching
accessors:
...
These should mirror the existing
...
- robust autocorrelation-aware null model with tests and clear
  constraints.

## Test Strategy

All new computational features should ship with explicit contracts and
at least four test families:
...
- grouped spectra recover known oscillations in simulated subject
  matrices,
...
- grouped results match direct per-subject calls where mathematically
  equivalent.
...
That gives `bosc` a general behavioral group-analysis layer quickly,
without locking in premature decisions about wavelet internals or AR1
