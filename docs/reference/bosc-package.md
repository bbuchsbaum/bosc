# bosc: Behavioral Oscillation Scores and Phase Analysis

R port of the behavioral-oscillations MATLAB toolbox, providing
functions for oscillation scoring, surrogate testing, phase analyses,
PPC metrics, clustering, and visualization for behavioral and neural
time series.

## References

Ter Wal, M., Linde-Domingo, J., Lifanov, J., Roux, F., Kolibius, L.,
Gollwitzer, S., Lang, J., Hamer, H., Rollings, D., Sawlani, V.,
Chelvarajah, R., Staresina, B., Hanslmayr, S., & Wimber, M. (2021).
Theta rhythmicity governs the timing of behavioural and hippocampal
responses in humans specifically during memory-dependent tasks. *Nature
Communications*, 12, 7048.
[doi:10.1038/s41467-021-25959-7](https://doi.org/10.1038/s41467-021-25959-7)

Original MATLAB toolbox:
<https://github.com/marijeterwal/behavioral-oscillations>

## See also

Useful links:

- <https://github.com/bbuchsbaum/bosc>

## Author

**Maintainer**: Bradley Buchsbaum <brad.buchsbaum@gmail.com>
